    ...
    for i=1:size(ground_truth_1,1)
        for k=1:size(ground_truth_2,1)
            ...
            for j=1:size(found_soma,2) 
                ...
                cond1 = (d1 < soma.maxRadius) && (d1<10 || pixelListBinarySearch(round(soma.pixelList),round(true_point_1)));
                cond2 = (d2 < soma.maxRadius) && (d2<10 || pixelListBinarySearch(round(soma.pixelList),round(true_point_2)));
               
                if (cond1 && cond2)
                    fp(j) = 0;
                elseif (bitxor(cond1,cond2))
                    fp(j) = -1;
                end
            end
        end
